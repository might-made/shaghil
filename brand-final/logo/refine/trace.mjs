...
// Morphological opening (erode then dilate) with a 3x3 structuring element — removes thin
// sub-resolution spikes/hairs (narrower than the erosion radius) while leaving the main
// silhouette's real edges and curves unchanged. Standard bitmap-cleanup technique, used here
// only when explicitly requested (openIterations > 0), never by default.
function erode(bin, width, height) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let dy = -1; dy <= 1 && keep; dy++) {
        for (let dx = -1; dx <= 1 && keep; dx++) {
          ...
          if (nx < 0 || ny < 0 || nx >= width || ny >= height || bin[ny * width + nx] !== 1) keep = false;
        }
      }
      out[idx] = keep ? 1 : 0;
    }
...
function dilate(bin, width, height) {
  const out = new Uint8Array(width * height);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const idx = y * width + x;
      ...
      for (let dy = -1; dy <= 1 && !any; dy++) {
        for (let dx = -1; dx <= 1 && !any; dx++) {
          const nx = x + dx, ny = y + dy;
          if (nx >= 0 && ny >= 0 && nx < width && ny < height && bin[ny * width + nx] === 1) any = true;
        }
      }
      out[idx] = any ? 1 : 0;
    }
...
function morphOpen(bin, width, height, iterations) {
  let cur = bin;
  for (let i = 0; i < iterations; i++) cur = erode(cur, width, height);
  for (let i = 0; i < iterations; i++) cur = dilate(cur, width, height);
  return cur;
}
