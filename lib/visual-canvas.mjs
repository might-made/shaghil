import {isolateProduct} from './background-removal.mjs';
...
export async function composeVisual(background,brand,settings,product=null,report={}){
  ...
  const panelHeight=settings.textMode==='none'?0:settings.textMode==='full'?h*0.36:h*0.26;
  const panelY=settings.textPosition==='top'?0:h-panelHeight;
  const contentTop=settings.textPosition==='top'?panelHeight+margin:margin;
  const contentBottom=settings.textPosition==='top'?h-margin:h-panelHeight-margin;
  const logoBottom=(settings.logoPosition||'top-right').startsWith('bottom');
  if(product?.fidelity==='exact'&&product.image){
    let productImage=product.image;
    try{productImage=await isolateProduct(product.image,product.id?product.id+':'+product.image.size:undefined)}catch{report.isolationFailed=true}
    const original=await decodeImage(productImage);
    const size={small:0.32,medium:0.5,large:0.68}[settings.productSize]||0.5;
    const logoSpace=brand.logo&&settings.logoVisible!==false?h*0.14:0;
    const top=contentTop+(logoBottom?0:logoSpace),bottom=contentBottom-(logoBottom?logoSpace:0);
  ...
  if(brand.logo&&settings.logoVisible!==false){
    ...
    const x=(settings.logoPosition||'top-right').endsWith('left')?margin:w-margin-logo.width*s;
    ctx.drawImage(logo,x,logoBottom?contentBottom-logo.height*s:contentTop,logo.width*s,logo.height*s);
  }
  if(settings.textMode!=='none'){
    ctx.fillStyle='rgba(9,12,15,0.88)';ctx.fillRect(0,panelY,w,panelHeight);
    ctx.fillStyle=brand.accent||brand.primary||'#e7f95b';ctx.fillRect(w-margin-80,panelY+margin/2,80,6);
    ctx.textAlign='right';ctx.textBaseline='top';ctx.direction='rtl';ctx.fillStyle='#ffffff';
    drawText(ctx,settings.headline,w-margin,panelY+margin,w-2*margin,3,52,panelHeight-margin*1.5-(settings.textMode==='full'&&settings.cta?100:0));
    if(settings.textMode==='full'&&settings.cta){ctx.fillStyle='#ffffff';drawText(ctx,settings.cta,w-margin,panelY+panelHeight-margin-45,w-2*margin,1,34)}
  }
