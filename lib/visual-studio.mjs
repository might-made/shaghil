...
function resultEntry(){el('visualEntry').classList.toggle('hidden',!['content','campaign','offer'].includes(current)||!lastText)}
function hideEntry(){el('visualEntry').classList.add('hidden')}
export function splitIdeas(text){
  // Keep original substrings intact so context can be checked by the server.
  let parts=text.split(/(?=^#{1,3}\s|^(?:\*\*)?(?:اليوم|يوم|Day)\s+[\d٠-٩])/m).map(s=>s.trim()).filter(Boolean);
...
async function choose(){
  ...
  try{source.brand={...DEFAULT,...await store.loadBrand()};status('studioBrand',`${source.brain.name} · ${source.brand.logo?'الشعار محفوظ':'بدون شعار'} · ${source.brand.references.length} صور مرجعية · ${source.brand.primary} / ${source.brand.secondary}`)}
...
function settings(prefix='visual'){return {format:el(prefix+'Format').value,mode:prefix==='visual'?el('visualMode').value:active.settings.mode,textMode:el(prefix+'TextMode').value,headline:el(prefix+'Headline').value.trim(),cta:el(prefix+'CTA').value.trim()}}
function validateSettings(s){if(s.textMode!=='none'&&!s.headline)throw new Error('راجع النص المعتمد قبل المتابعة')}
async function payload(snapshot){return {brain:snapshot.brain,brand:{...snapshot.brand,logo:snapshot.brand.logo?await blobPayload(snapshot.brand.logo):null,references:await Promise.all(snapshot.brand.references.map(blobPayload))},task:snapshot.task,settings:snapshot.settings,previousDirection:snapshot.previousDirection??-1}}
async function generate(){
  if(visualBusy||!source?.brand)return;
  const s=settings();
  try{validateSettings(s)}catch(e){return status('visualStatus',e.message)}
  return requestVisual({...structuredClone(source),settings:s});
}
async function requestVisual(snapshot){
  ...
  try{
    ...
    try{record.rendered=await composeVisual(background,record.brand,record.settings)}catch(e){record.settings={...record.settings,textMode:'none'};try{record.rendered=await composeVisual(background,record.brand,record.settings);toast(e.message+' — حُفظت نسخة بدون نص')}catch{record.rendered=background;record.overlayWarning='حُفظت الصورة الأصلية؛ تعذّر تركيب الشعار والنص. أعد التعديل بدون توليد جديد.'}}
...
async function variant(kind){
  ...
  await requestVisual({brain:structuredClone(active.brain),brand:structuredClone(active.brand),task:{...active.task},inputs:active.inputs,settings:s,previousDirection:active.direction});
}
...
function editOverlay(){if(!active||visualBusy)return;for(const [key,id]of [['format','editFormat'],['textMode','editTextMode'],['headline','editHeadline'],['cta','editCTA']])el(id).value=active.settings[key];el('overlayEditor').classList.remove('hidden')}
async function updateOverlay(s){
  if(visualBusy||!active)return;
  lock(true);
  try{validateSettings(s);const record={...active,overlayWarning:'',settings:s,rendered:await composeVisual(active.background,active.brand,s)};let saved=true;try{await store.saveVisual(record)}catch{saved=false}display(record);status('visualResultStatus',saved?'تم التعديل محليًا بدون تكلفة توليد جديدة.':'تم التعديل؛ تعذّر الحفظ. حمّل التصميم الآن.')}
...
globalThis.Visual={setupBrand,saveProject,brandSummary,uploadLogo:file=>upload(file?[file]:[],true),uploadReferences:files=>upload(Array.from(files),false),removeLogo(){if(brandDraft&&!uploadBusy){brandDraft.logo=null;previewAssets()}},removeReferences(){if(brandDraft&&!uploadBusy){brandDraft.references=[];previewAssets()}},resultEntry,hideEntry,choose,selectIdea,textControls,generate,variant,back,download,editOverlay,applyOverlay:()=>updateOverlay(settings('edit')),noText:()=>active&&updateOverlay({...active.settings,textMode:'none'}),history};
