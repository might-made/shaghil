...
function lock(value){visualBusy=value;globalThis.Packs?.setGenerationBusy(value);el('visualGenerate').disabled=value;document.querySelectorAll('#visualActions button, #aiActions button').forEach(b=>b.disabled=value)}
...
async function variant(kind){
  ...
  await requestVisual({parentId:active.id,rootId:active.rootId||active.id,action:kind,product:active.product?structuredClone(active.product):null,brain:structuredClone(active.brain),brand:structuredClone(active.brand),task:{...active.task},inputs:active.inputs,settings:s,previousDirection:active.direction});
}
...
async function history(){
  ...
  try{
    ...
    rows.forEach(record=>{const button=document.createElement('button');button.className='historyItem btn full';button.textContent=`${record.brain?.name||'مشروع'} · ${record.parentId?'معالجة':'تصميم'} · ${titles[record.task.engine]} · ${record.settings.format} · ${record.settings.mode} · ${plain(record.task.selected).slice(0,90)}`;button.onclick=()=>{if(visualBusy)return toast('انتظر اكتمال التصميم الحالي');display(record)};list.appendChild(button)});
