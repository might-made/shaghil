...
async function upload(file){
  ...
  try{if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>8*1024*1024)throw new Error('اختر PNG أو JPEG أو WebP أقل من 8MB');const image=await decodeImage(file);if(image.width*image.height>40000000)throw new Error('أبعاد الصورة كبيرة جدًا');const reference=await normalizeAsset(file);pending={image:file,reference};showPreview(file);message(`${file.name} — الصورة جاهزة. اضغط "حفظ المنتج / Save Product" لإتمام الحفظ`)}
